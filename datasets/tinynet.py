...
class TinyImageNet200Idx(TinyImageNet200):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        # Extract targets from data_info for consistency with CIFAR10_idx interface
        self.targets = [item[1] for item in self.data_info]
...
def get_tinynet_loaders_idx(config):
    train_dataset = TinyImageNet200Idx(
    ...
    test_dataset = TinyImageNet200Idx(
        root=config.data_dir,
        train=False,
        download=True,
        transform=train_transform
